...
<template>
    <div class="zdtb">
        ...
        <Row>
            <Col span="10" class="mt10">共{{$_total_$}}条数据</Col>
            <Col span="2" offset="12">
                <Button type="primary" @click="$_addRole_$" class="mt10" style="float: right">+ 新增角色</Button>
                <!-- 创建角色 -->
                <Modal v-model="$_modelRole_$" title="角色信息" :mask-closable="false">
                    ...
                    <div slot="footer" align="center">
                         <center><Button type="primary" size="large" @click="$_createNew_$">保存</Button></center>
                        <!-- <Button size="large" @click="$_quit_$">取消</Button>
                        <Button type="primary" size="large" @click="$_createNew_$" >保存</Button> -->
                    </div>
        ...
        <Modal v-model="$_rmModelEdit_$" title="角色信息" :mask-closable="false">
           ...
           <div slot="footer" align="center">
                         <center><Button type="primary" size="large" @click="$_bjNew_$">保存</Button></center>
                        <!-- <Button size="large" @click="$_quit_$">取消</Button>
                        <Button type="primary" size="large" @click="$_createNew_$" >保存</Button> -->
                    </div>
        </Modal>
        ...
        <Row class="mt10">
            ...
            <Modal v-model="$_treeSetting_$" title="权限设置" :mask-closable="false">
                ...
                <div slot="footer">
                    <center><Button type="primary" size="large" @click="handleEditRights">保存</Button></center>
    ...
    export default {
        ...
        methods: {
            ...
            $_List_$() {
                this.$_sendQuery_$({
                    method: "GET",
                    url: this.$_global_$.serverPath + `/user/role/enterprise/`+this.userInfo.enterpriseId,
                    data: {},
                    headers: {
            ...
            $_createNew_$() {
                this.$refs.$_newRolev_$.validate((valid) => {
                    if (valid) {
                        ...
                        this.$_sendQuery_$({
                            method: "POST",
                            url: this.$_global_$.serverPath + `/user/role/create/enterprise/`+this.userInfo.enterpriseId,
                            data:{
                    ...
                    if (rsp.status === 200) {
                        if (rsp.data.code === 0) {
                            ...
                            this.$_sendQuery_$({
                                ...
                                if (rsp.status === 200) {
                                    if (rsp.data.code === 0) {
                                        for (let i = 0; i < rsp.data.data.company.length; i++) {
                                            this.$_bumen_$.push(rsp.data.data.company[i]);
                                            if (rsp.data.data.company[i].child) {
                                                this.$_hy_$(rsp.data.data.company[i]);
                                            }
                                        }
