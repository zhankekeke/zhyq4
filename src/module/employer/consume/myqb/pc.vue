...
<template>
    <div class="container">
        <Row class="top mes" v-model="$_Mes_$">
            <Row style="font-size: 16px;"><strong>账户信息</strong></Row>
            <Row>
                <Col span="6">
                    账户名称：
            ...
            <Row class="three">
                ...
                <Col span="6">
                    积分账户：
                    <span>{{$_Mes_$.credits}}</span>积分
                    <!-- <a href="javascript:void(0)" @click="$_jfzzBtn_$">积分转赠</a> -->
                </Col>
                <Col span="6">
                    优惠券：
                    <span class="color1" style="cursor: pointer" @click="Effectivecoupon">{{couponnum}}</span> 张可用
                </Col>
            </Row>
            <Row>
                <Col span="6">
                    <Button type="primary" shape="circle" @click="$_xfTypeFun_$('cash')">钱包账单</Button>
                </Col>
                <Col span="6">
                    <Button type="ghost" shape="circle" @click="$_xfTypeFun_$('credit')">积分账单</Button>
                </Col>
            </Row>
        </Row>
        ...
        <Row style="background: #fff;padding: 25px">
            <Table :columns="$_xfType_$ == 'cash'?$_columns_$:$_columns2_$" :data="$_data_$"></Table>
        </Row>
        <Row class="margin">
            <Col span="4" style="text-align: left">共有{{$_num_$}}条数据</Col>
            <Page
                    :total="$_num_$"
                    show-elevator
                    show-sizer
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            Effectivecoupon(){
                this.$root.$_Route_$("employer", "jump", "kyyhq")
            },
            // 获取可用优惠券
            Coupon() {
                this.$_sendQuery_$({
