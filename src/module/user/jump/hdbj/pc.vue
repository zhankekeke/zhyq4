    ...
    <div>
        <!-- <Form class="wrap" :rules="formValidate" ref="form" :model="$_Form_$" :label-width="100">
            <FormItem label="活动名称" prop="title">
                <Input v-model="$_Form_$.title"></Input>
            </FormItem>
            <FormItem label="活动地址" prop="address">
                <Input v-model="$_Form_$.address"></Input>
            </FormItem>
            <FormItem label="活动介绍" prop="content">
                <Input v-model="$_Form_$.content"></Input>
            </FormItem>
            <FormItem label="起止时间" prop="date">
                <DatePicker v-model="$_Form_$.date"  @on-change = "startTimeChange" type="datetimerange"style="width:500px"placeholder="Select date"
                format = "yyyy-MM-dd HH:mm:ss"
                :start-date="new Date(new Date().getFullYear(),new Date().getMonth()+1, new Date().getDate())"
                ></DatePicker>
            </FormItem>
            <Row>
                <Col span="12">
                ...
                <Col span="12">
                    <FormItem label="人数限制" prop="peopleNumber">
                        <Input v-model="$_Form_$.peopleNumber"></Input>
                    </FormItem>
                </Col>
            </Row>
            <Row>
                <Col span="12">
                    <FormItem label="联系人" prop="contactName">
                        <Input v-model="$_Form_$.contactName"></Input>
                    </FormItem>
                </Col>
                <Col span="12">
                    <FormItem label="联系方式" prop="contactNumber">
                        <Input v-model="$_Form_$.contactNumber"></Input>
                    </FormItem>
                </Col>
            </Row>
            <Row>
                <Col span="12">
                    <FormItem label="发放代金券">
                        <Select v-model="$_Form_$.couponId" style="width:200px">
                            <Option value="0">代金券一</Option>
                            <Option value="1">代金券二</Option>
                            <Option value="2">代金券三</Option>
                        </Select>
                    </FormItem>
                </Col>
                <Col span="12">
    ...
                    accept="image/png,image/gif,image/jpg,image/jpeg">
            ...
            <Row>
                <Col span="12">
                    <FormItem label="发放代金券">
                        <Select v-model="$_Form_$.couponId" style="width:200px">
                            <Option v-for="item in voucherList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                        </Select>
                    </FormItem>
                </Col>
                <Col span="12">
        ...
        endTimeOptions: {}, //结束日期设置
          ...
          formValidate: {
            title: [
                ...
                {type: 'string', max:100, message:'活动名称不能超过100个字', trigger:'blur'},
                {validator: this.$_validatestr_$, trigger:'change'}
            ],address: [
                { required: true, message: '地址不能为空', trigger: 'blur' },
                {type: 'string', max:100, message:'地址不能超过100个字', trigger:'blur'},
                {validator: this.$_validatestr_$, trigger:'change'}
            ],peopleNumber: [
...
      },
     $_edit_$() {
        this.$_Form_$ = this.$root.inparams.data;
        this.$_Form_$.apply = ''+this.$_Form_$.apply
        this.$_Form_$.peopleNumber = ''+this.$_Form_$.peopleNumber
        this.$_file_$ = JSON.parse(this.$_Form_$.images)
        this.files = this.$_Form_$.files.split(",")
        if (this.$_file_$.length > 0) {
                    for (let i = 0; i < this.$_file_$.length; i++) {
                        this.imageList.push({
                            url: this.$_file_$[i]
                        })
                        this.$_file_$.push(this.row.images[i])
                    }
                }
   ...
   $_save_$() {
       this.$set(this.$_Form_$, 'images',JSON.stringify(this.$_file_$))
        ...
        if(this.$_Form_$.couponId){
            this.$_Form_$.couponId
        }else{
            this.$_Form_$.couponId = ''
        }
                ...
                this.$_sendQuery_$({
                    ...
                    data:{
                       ...
                       "beginDate": this.$_Form_$.beginDate,
                        ...
                        "apply": Number(this.$_Form_$.apply),
                        "peopleNumber": Number(this.$_Form_$.peopleNumber),
                        ...
                        "couponCount": Number(this.$_Form_$.couponCount),
